...
3. Run a failing command through FixTape capture.
...
fixtape finish --verdict fixed --summary "Retry path now checks idempotency key" --include-last 20m
fixtape show
...
- Late session starts are recoverable because the flight recorder can promote recent debugging history.
